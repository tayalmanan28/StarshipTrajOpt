...
def x_dot(x, u):
    ...
    # Horizontal force
    F_x = max_thrust * thrust * np.sin(thrust_angle + theta)
    x_dot = x[1]
    x_dotdot = (F_x) / m
        
    # Vertical force
    F_y = max_thrust * thrust * np.cos(thrust_angle + theta)
    y_dot = x[3]
    y_dotdot = (F_y) / m - g
    
    # Torque
    T = -length/2 * max_thrust * thrust * np.sin(thrust_angle)
    theta_dot = x[5]
    theta_dotdot = T / I
...
def update(i):
  ...
  rocket_x = x_t[i, 0]
  rocket_x_points = [rocket_x + length/2 * np.sin(rocket_theta), rocket_x - length/2 * sin(rocket_theta)]

  rocket_y = x_t[i, 2]
  rocket_y_points = [rocket_y + length/2 * np.cos(rocket_theta), rocket_y - length/2 * cos(rocket_theta)]
...
anim.save('spaceshiplanding.mp4', writer = 'ffmpeg', fps = 30)